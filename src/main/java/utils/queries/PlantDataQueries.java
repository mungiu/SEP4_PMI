...
public class PlantDataQueries {
    ...
    public static final String CREATE_PLANT_DATA = "insert into SEP4_PMI.dbo.PlantInfo(Plant_ID, Sensor_Type, Sensor_Value, TimeStamp) values (?,?,?,?);";/*CURRENT_TIMESTAMP instead of the last ? If you want the stamp from the Microsoft SQL server. Else retreive before on java server or device.*/
}
